...
sap.ui.define([
	...
	function ModelerProxy(serviceConfiguration, inject){
		...
		function readConfigurationList(callback, filter){
			...
			var uriForReadingConfiguration = textFileAndAnalyticalConfigurationsUri + "?$select=AnalyticalConfiguration,AnalyticalConfigurationName,SerializedAnalyticalConfiguration&$filter=" + filter.toUrlParam();
			ajax({
				type: "GET",
		...
		this._updateRemoteTextFile = function(applicationId, textFileLanguage, textFile, isTextPoolCleanup) {
			var promise = new Promise(function(resolve, reject){
				...
				if(isTextPoolCleanup === true) {
					...
					uriForUpdateTextPropertyFile = textFilesUri + '/' + applicationId;
				} else {
					// Called when executing import from file system
					uriForUpdateTextPropertyFile = textFilesUri + '/' + applicationId + '/' + textFileLanguage;
				}
				var data = {
