...
sap.ui.define([
	...
	var constructor = sap.ui.core.mvc.Controller.extend("sap.apf.modeler.ui.controller.propertyType", {
		...
		setDetailData : function() {
			var deferred = jQuery.Deferred();
			var oController = this;
			// FIXME optionsValueModelBuilder is a module for static methods, remove as parameter.
		...
		handleChangeForPropertyTypeAsPromise : function(event) {
			...
			//oController.getView().fireEvent(oConstants.UPDATEPROPERTYVALUESTATE, { // FIXME finally remove handler method, event binding, constant and test.
			//	"sProperty" : sSelectedPropertyName
			//}); FIXME fireEvent is async, therefore exec is not synchronous here.
				...
				oPropertyOrchestration.updateAllSelectControlsForPropertyType(viewId, sSelectedPropertyName).then(function(){
					...
					oController.updateRepresentationAndView().then(function() {
						if (isSwapCase){
							...
							dependentController.updateRepresentationAndView().then(function() { // FIXME argument dependentSelectedPropertyName is unnecessary
		...
		removeAllItemsFromDropDownList : function() { // FIXME test prove that removeItem is called as many times as length of getItems()
			var oController = this;
		...
		/**
		 * Building a new dropDownList. FIXME "none" and "not-available".
		 ...
		 */
		//FIXME add tests for "none" option and "Not-Available" prefix addition
