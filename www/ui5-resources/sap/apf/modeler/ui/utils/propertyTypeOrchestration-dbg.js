...
sap.ui.define([
	 ...
	 * @param {{available:String[], consumable:String[]}}[] filteredProperties
	 * @constructor
	 */
	function PropertyTypeOrchestration(filteredProperties){
		var propertyTypeRows = [];
		this._getPropertyTypeRows = function(){ // public for test design
			return propertyTypeRows;
		};
		this.getAvailablePropertyNames = function(){ // for test only
		...
		this.addPropertyTypeReference = function(controlId, propertyRowInformation, sPropertyType, oView){
			propertyTypeRows.push({
				controlId : controlId,
				propertyRowInformation : propertyRowInformation, // FIXME like this: JSON.parse(JSON.stringify(propertyRowInformation)),
		 ...
		 * FIXME not optional replace usage in add/remove ... Function parameters are optional; meaning that the function will by default update all the property type rows managed by the orchestration object.
