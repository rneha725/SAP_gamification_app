				...
				propertyTypeOrchestration.getConsumableAndAvailablePropertiesAsPromise(representationId, modelerUiConstants.aggregationRoles.MEASURE, basicDataHandler.oStepPropertyMetadataHandler)
					.then(function(aMeasures){
						...
						if (basicDataHandler.oRepresentation.getRepresentationType() === "TreeTableRepresentation") {
							oPropertyOrchestrationProperties = new propertyTypeOrchestration.PropertyTypeOrchestration(aDimensionsAndLegends); // FIXME  what needs to be constructed?
							...
							oPropertyOrchestrationDimensionsLegends = new propertyTypeOrchestration.PropertyTypeOrchestration(aDimensionsAndLegends);
							...
							oPropertyOrchestrationMeasures = new propertyTypeOrchestration.PropertyTypeOrchestration(aMeasures);
							_instantiateMeasureView(basicDataHandler, oPropertyOrchestrationMeasures);
						}
						promiseMetadataAndConfigurationLoaded.resolve();
					});
			});
		return jQuery.when.apply(jQuery, this.propertyTypeHandlerPromises); // FIXME new Promise.all(..)
	};
