sap.ui.define(["./AxisTimeStrategyBase", "sap/gantt/misc/Format"], function (AxisTimeStrategyBase, Format) {
	...
	StepwiseZoomStrategy.prototype.init = function() {
		...
		this.setProperty("zoomLevel", -1, true);
		this._mappedZoomLevel = -1;
	...
	StepwiseZoomStrategy.prototype.applySettings = function () {
		...
		this._initialTotalHorizon = new sap.gantt.config.TimeHorizon({
			startTime: this.getTotalHorizon().getStartTime(),
			endTime: this.getTotalHorizon().getEndTime()
		});

		//Considering customized zoomLevel or timeLineOption
		if (this.getZoomLevel() === -1 ){
			this._bInitialZoomLevel = false;
			this.setProperty("zoomLevel", 0, true);
			// if (!this.getTimeLineOption()) {
			// 	this.setProperty("timeLineOption", this._getTimeLineOptionByIndex(0), true);
			// }
		} else {
			this._bInitialZoomLevel = true;
			this._nInitialZoomLevel = this.getZoomLevel();
		}
	...
	StepwiseZoomStrategy.prototype._updateZoomControlType = function (sZoomControlType) {
		if (sZoomControlType) {
			...
			this._mapZoomLevelByZoomType(sZoomControlType);
			this._updateZoomRateArray();
			this._updateTimeLineOptions();

			if (this._bInitialZoomLevel) {
				this.setZoomLevel(this._mappedZoomLevel);
			} else {
				//if initial zoomLevel is not set but timelineOption is set
				this.setZoomLevel(this._getIndexOfTimeLineOption(this.getTimeLineOption(), this.getTimeLineOptions()));
			}
		}
	};
	...
	StepwiseZoomStrategy.prototype._mapZoomLevelByZoomType = function(sZoomControlType) {
		if ( this._bInitialZoomLevel && this._mappedZoomLevel === -1 ) {
			if (sZoomControlType !== sap.gantt.config.ZoomControlType.Select){
				this._mappedZoomLevel = this._getIndexOfTimeLineOption(this.getTimeLineOption(), this.getTimeLineOptions());
			} else {
				this._mappedZoomLevel = this._getCountOfTimeLineOptions() - 1 - this._getIndexOfTimeLineOption(this.getTimeLineOption(), this.getTimeLineOptions());
			}
		}
	};
