...
sap.ui.define(["jquery.sap.global", "sap/ovp/cards/charts/Utils", "sap/ui/model/odata/CountMode",
        ...
        "sap/viz/ui5/data/MeasureDefinition", "sap/viz/ui5/controls/common/feeds/FeedItem", "sap/ui/core/Element"],
    function (jQuery, Utils, CountMode, CardAnnotationHelper, NumberFormat, DateFormat,
              ...
              MeasureDefinition, FeedItem, CoreElement) {
        "use strict";
...
//										showScale: false,
        ...
        function buildSemanticLegends(oMeasure, entityTypeObject, oMetadata) {
            function endsWith(sString, sSuffix) {
                return sString && sString.indexOf(sSuffix, sString.length - sSuffix.length) !== -1;
            }
            ...
            if (oMetadata[measureName]) {
                if (oMetadata[measureName][constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                    measureName = oMetadata[measureName][constants.LABEL_KEY_V4].String ? oMetadata[measureName][constants.LABEL_KEY_V4].String : oMetadata[measureName][constants.LABEL_KEY_V4].Path;
...
//		}
            ...
            if (config.properties && config.properties.semanticColor === true && checkFlag(aMeasures, entityTypeObject)) {
                ...
                if (aMeasures.length === 1) {
                    var legends = buildSemanticLegends(aMeasures[0], entityTypeObject, oMetadata);
            ...
            if ((chartContext.ChartType.EnumMember === constants.COLUMNSTACKED_CHARTTYPE || 
                    chartContext.ChartType.EnumMember === constants.DONUT_CHARTTYPE) && 
                    aColorPalette && aColorPalette.length === 4) {
                ...
                if (colorPaletteDimension) {
                    ...
                    //Dimension Sorting to be done using PreVar
                    /*if (chartContext.ChartType.EnumMember === constants.COLUMNSTACKED_CHARTTYPE) {
...
//		var minFractionDigits = Number(dataContext.NumberFormat.NumberOfFractionalDigits.Int);
                            ...
                            if (oMetadata && oMetadata[property]) {
                                if (oMetadata[property][constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                                    if (oMetadata[property][constants.LABEL_KEY_V4].String) {
                                        feedName = oMetadata[property][constants.LABEL_KEY_V4].String;
            ...
            if (isEDMINT32 && !isFractionalDigitsSet) {
                ...
                if (oVizProperties && oVizProperties["valueAxis"]) {
        ...
        function setChartUoMTitle(vizFrame, vizData, chartTitle) {
            ...
            if (result) {
                jQuery.each(measureList, function (i, property) {
                    ...
                    if (oMetadata && oMetadata[property]) {
                        if (oMetadata[property][constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                            feedName = oMetadata[property][constants.LABEL_KEY_V4].String ? oMetadata[property][constants.LABEL_KEY_V4].String : oMetadata[property][constants.LABEL_KEY_V4].Path;
                        } else if (oMetadata[property][constants.LABEL_KEY]) {
                            feedName = oMetadata[property][constants.LABEL_KEY];
                        } else if (property) {
                            feedName = property;
