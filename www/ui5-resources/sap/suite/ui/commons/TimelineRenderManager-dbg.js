...
sap.ui.define("sap/suite/ui/commons/TimelineRenderManager", [
	...
	ViewSettingsCustomItem, OverflowToolbarLayoutData, OverflowToolbarPriority, MessageToast, InvisibleText, SliderTooltip, TimelineRenderManagerTimestamp) {
	"use strict";
		...
		extendTimeline: function (Timeline) {
			...
			Timeline.prototype._setupRangeFilterPage = function () {
				...
				this._objects.register("timestampFilterPicker", function () {
					return new TimelineRenderManagerTimestamp(that.getId(), undefined, undefined, resourceBundle);
				});
						...
						oFilterDialog = new ViewSettingsDialog(that.getId() + "-filterContent", {
							confirm: function (oEvent) {
								...
								if ( that._objects.getTimestampFilterPicker().getVisible() ) {
									that._startDate = that._objects.getTimestampFilterPicker().getStartDate();
									that._endDate = that._objects.getTimestampFilterPicker().getEndDate();
									bRange = true;
								} else {
