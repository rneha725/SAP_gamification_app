...
function(FilterItem, Sorter, oData4Analytics, FilterUtil, NumberFormat) {
	"use strict";
	...
	FilterItemChart.prototype._bFirstTimeRender = false;
	FilterItemChart.prototype.init = function() {
		this._bAllowBindingUpdateOnPropertyChange = false;
		this._attachChartEvents();
	};

	/*onAfterRendering is called when the rendering of the control is completed
	Here we are tring to make sure to call the _updateBinding only when the control is visible
	(in our case when user clicks on the Visual filter) & make sure the lazyLoadVisualFilterbar is true coming from manifest*/
	FilterItemChart.prototype.onAfterRendering = function() {
		if (!this._bFirstTimeRender && this.getLazyLoadVisualFilter()) {
			this._bFirstTimeRender = true;
			this._updateBinding();
		}
	};
	...
	FilterItemChart.prototype.checkSearchable = function(oDataModel, oEntitySet, oSmartFilterBar, aParameters, aSVOptions) {
		...
		if (parameterization) {
			...
			if (oSmartFilterBar.getEntitySet() === oEntitySet) {
				...
				allMandatoryParameters.forEach(function(mParam) {
					oSmartFilterBar.getFiltersWithValues().forEach(function(param) {
						if (param.getName().indexOf(mParam) !== -1 && allParamsList.indexOf(param.getName()) < 0) {
