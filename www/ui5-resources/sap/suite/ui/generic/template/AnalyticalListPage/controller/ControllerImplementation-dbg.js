...
sap.ui.define(["jquery.sap.global", "sap/ui/base/Object", "sap/ui/model/json/JSONModel", "sap/m/ObjectIdentifier", "sap/m/Table",
		...
		"sap/m/OverflowToolbar", "sap/m/ToolbarSpacer", "sap/m/library"
	],
    function(jQuery, BaseObject, JSONModel, ObjectIdentifier, Table, Text, SmartField, SelectionVariant, EventProvider, ExtensionAPI, ResizeHandler,
			...
			ODataAnnotationHelper, Analytics, ContentAreaController, IappStateHandler, FilterUtil, Device, SegmentedButtonItem, SegmentedButton, OverflowToolbar, ToolbarSpacer, SapMLibrary) {
		"use strict";
				...
				return {
					onInit: function() {
						...
						if (oState.alr_visualFilterBar) {
							oState.alr_visualFilterBar.attachInitialized(function(oEvent) {
								var bIsStandardVariant = oState.oSmartFilterbar.isCurrentVariantStandard();
								...
								if (bIsStandardVariant && !oState.oSmartFilterbar.isDialogOpen() && oTemplatePrivateModel.getProperty("/alp/filterMode") === FILTER_MODE_VISUAL && oTemplatePrivateModel.getProperty("/alp/searchable") === false) {
								...
								if (oState.oIappStateHandler._bVisualFiltersUpdated === false) {
					...
					handlers: {
						...
						onBeforeSFBVariantSave: function() {
							 ...
							 * context is needed!
							 */
							if (oState.oSmartFilterbar.isDialogOpen()) { //on click of save in the dialog, update the filterbar as well without waiting for the user to confirm/cancel by clicking go/cancel
