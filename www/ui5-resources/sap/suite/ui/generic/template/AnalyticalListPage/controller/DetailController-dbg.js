		...
		var tController = Controller.extend("sap.suite.ui.generic.template.AnalyticalListPage.controller.DetailController", {
			...
			_onBeforeRebindTable: function(oEvent) {
				var variant = this.oState.oSmartTable.fetchVariant(),
					lastVariant = variant,
					oTable = this.oState.oSmartTable.getTable(),
					changeObject = {},
				...
				var filtersFromChartDrilldown = this.oState.oSmartChart ? this.oState.oSmartChart.getChart().getDrillStack().pop() : undefined;
				...
				if (variant.sort && variant.sort.sortItems) {
					for (var i = 0; i < variant.sort.sortItems.length; i++) {
						...
						sortList.push(new Sorter(variant.sort.sortItems[i].columnKey, isDescending));
					}
				...
				if ((!variant || !variant.sort) && oTable instanceof ResponsiveTable && this.oState.oSmartTable.data("TemplateSortOrder")) {
					var aSortOrder = this.oState.oSmartTable.data("TemplateSortOrder").split(", ");
				...
				if (oTable instanceof ResponsiveTable) {
						...
						for (var iColumn = 0; iColumn < oColumns.length; iColumn++) {
							for (var iCustomData = 0; iCustomData < oColumns[iColumn].getCustomData().length; iCustomData++) {
								...
								if (oCustomData.getProperty("key") === "p13nData" && oCustomData.getValue("p13nData").leadingProperty && oCustomData.getValue("p13nData").leadingProperty === oSorter.sPath) {
									...
									oSorter.fnGroup = this.fnGroupFunction(oSorter.sPath, sColumnLabel);
									return;
								}
							}
						}
			...
			_applyChartSelectionOnTableAsFilter: function(oEvent) {
				...
				var andFilter = [],
				chart = this.oState.oSmartChart.getChart();
				if (!chart) {
					return;
				}
				var paramList = this._getSelParamsFromChart(chart);
			...
			/**
			 * For finding the right key and label for the grouping row of a table
			 * @param  {string} sPath Sorter.sPath
			 * @param  {string} sColumnLabel
			 * @return {object} key and label for grouping
			 */
			fnGroupFunction: function(sPath, sColumnLabel) {
				var sText = "";
				var mTypeMap = []; // buffer to take care for subsequent calls
								...
								if (oProperty.extensions[k].namespace === "http://www.sap.com/Protocols/SAPData") {
									switch (oProperty.extensions[k].name) {
										case "display-format":
											...
											break;
										case "label":
											if (!sLabel) {// If sColumnLabel is undefined.
