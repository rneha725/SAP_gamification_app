sap.ui.define(["jquery.sap.global", "sap/ui/base/Object", "sap/ui/base/Event", "sap/ui/core/mvc/ControllerExtension", "sap/ui/Device", "sap/ui/model/Context", "sap/ui/model/Filter", "sap/ui/model/json/JSONModel",
	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		*/
		...
		function fnSetAnalyticalBindingPath(oSmartFilterBar, fnSetBindingPath){
			// still open
			...
			if (oSmartFilterBar && oSmartFilterBar.getAnalyticBindingPath && oSmartFilterBar.getConsiderAnalyticalParameters()) {
				...
				try {
					...
					jQuery.sap.log.warning("Mandatory parameters have no values", "", "ListReport");
				}
			}				
		}
		...
		function onBeforeRebindTableOrChart(oEvent, oCallbacks, oSmartFilterBar){
			...
			var fnPerformExtensionFunction = function(sExtensionName, fnExtensionCallback){
				if (oCallbacks && oCallbacks[sExtensionName]){
					...
					oCallbacks[sExtensionName](fnPerformExtensionCallback);
					bIsAllowed = false;
				}
			};
			...
			fnPerformExtensionFunction("addExtensionFilters", fnAddFilter);
			// End Filter handling
			
			fnSetAnalyticalBindingPath(oSmartFilterBar, oCallbacks.setBindingPath);
			...
			var fnEnsureSelectionProperty = function(sProperty){
				if (sProperty && jQuery.inArray(sProperty, aSelects) === -1) {
					aSelects.push(sProperty);
				}
			};
			...
			fnPerformExtensionFunction("ensureExtensionFields", fnEnsureSelectionProperty);
