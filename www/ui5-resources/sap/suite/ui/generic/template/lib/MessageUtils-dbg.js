...
sap.ui.define(["sap/ui/generic/app/util/ModelUtil", "sap/ui/generic/app/util/MessageUtil", "sap/m/MessageBox"
	...
	function(ModelUtil, GenericMessageUtil, MessageBox) {
		...
		function fnNavigateToMessageTarget(oScrollDelegate, oMessage) {
			var sId = oMessage.getControlId();
			var oTarget = sId && sap.ui.getCore().byId(sId);
			if (oTarget) {
				if (oScrollDelegate){
		...
		function fnNavigateFromMessageViewEvent(oScrollDelegate, oEvent) {
			var oItem = oEvent.getParameter("item");
			var oMessage = oItem.getBindingContext("msg").getObject();
			fnNavigateToMessageTarget(oMessage, oScrollDelegate);
		}
		...
		return {
			...
			navigateFromMessageViewEvent: fnNavigateFromMessageViewEvent,
