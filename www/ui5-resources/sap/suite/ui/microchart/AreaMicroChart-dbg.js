			...
			for (var i = 0; i < aLabelContainer.length; i++) {
				...
				for (var j = 0; j < $Labels.size(); j++) {
					if (this._isLabelTruncated($Labels[j])) {
						if ($Labels.eq(j).hasClass("sapSuiteAMCPositionCenter")) {
							// hide min/max values if one of them is truncated
							$TopLabelContainer.find(".sapSuiteAMCPositionCenter").css("visibility", "hidden");
							$BottomLabelContainer.find(".sapSuiteAMCPositionCenter").css("visibility", "hidden");
						} else {
							// hide whole top/down container if one of first/last labels is truncated
