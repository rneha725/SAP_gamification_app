		...
		aggregations: {
			...
			items: {
				propagateMetadata: function(oElement) {
					...
					// Check if element is related to SmartLink (allow both SmartLink and ObjectIdentifier for RTA) or Toolbar (also OverflowToolbar)
					var bIsSmartLinkOrToolbar = (sType === "sap.ui.comp.navpopover.SmartLink" || sType === "sap.m.ObjectIdentifier" || sType === "sap.m.Toolbar" || sType === "sap.m.OverflowToolbar");
					// Disable RTA for all other UI5 table related code and sap.m.Title
					if (!bIsSmartLinkOrToolbar && (sType === "sap.m.Title" || fCheckTableTypesMatch(sType) || fCheckTableTypesMatch(oElement.getParent().getMetadata().getName()))) {
						return {
							actions: null
						};
					} else if (sType === "sap.m.Toolbar" || sType === "sap.m.OverflowToolbar"){
						return {
							actions: {
								addXML : {
									jsOnly : true
