...
// }));
	...
	FilterBar.prototype._checkForCollision = function(oTitleElement, oLabelElement) {
		...
		if (sap.ui.getCore().getConfiguration().getRTL()) {
			if ((this._oLabelTextWidth > 0) && oResizeDomRef && ((oResizeDomRef.clientWidth - this._oLabelTextWidth - 32) < oTitleElement.position().left)) {
				return false;
			}
		} else {
	...
	FilterBar.prototype._showFilterDialog = function() {
		...
		this._oFilterDialog.isPopupAdaptationAllowed = function() { return false; };
	...
	function _DialogLoaded(fnDialog, fnOverflowToolbarLayoutData) {
...
// }
	...
	FilterBar.prototype._generateListItems = function(sGroupName, oList) {
		...
		if (!CustomListItem) {
			...
			if (!CustomListItem) {
				throw new Error("_generateListItems msut only be called via _createAddRemoveFiltersDialog " + this);
			}
		}
	...
	FilterBar.prototype._applyVariant = function(oVariant, sContext, bInitial) {
		...
		if (oVariant) {
			...
			this._applyVariantFields(aFieldsAndValues); // BCP: 1880228255
