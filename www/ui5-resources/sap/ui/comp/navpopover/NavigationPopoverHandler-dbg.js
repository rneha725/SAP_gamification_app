...
sap.ui.define([
	...
	NavigationPopoverHandler.prototype._initModel = function() {
		...
		if (!oControl) {
			jQuery.sap.log.error("sap.ui.comp.navpopover.NavigationPopoverHandler: No control provided, popover cannot be attached.");
		}
	...
	NavigationPopoverHandler.prototype._calculateSemanticAttributes = function(oSemanticObjects) {
		...
		aSemanticObjects.forEach(function(sSemanticObject) {
			...
			var oMappingRules = that._getMappingRules(sSemanticObject, oSemanticObjects, oContext);
			...
			for ( var sAttributeName in oContext) {
				...
				if (oResults[sSemanticObject][sAttributeNameMapped]) {
					if (oContext[sCurrentField]) {
						// Take over the value of current field in case of clash. If other field has clash we have no clue which value is the right one. So we log it out.
						// Keep in mind: we do not explicitly check whether we are in the 'mapping' use-case when determining oMappingRules because in not 'mapping'
	...
	NavigationPopoverHandler.prototype._getMappingRules = function(sSemanticObject, oSemanticObjects, oContext) {
		...
		for (sAttributeName in oContext) {
