...
sap.ui.define(['jquery.sap.global', 'sap/ui/comp/library', 'sap/ui/core/Control', './FieldSelectorController',
               'sap/m/library', 'sap/m/HBox', 'sap/m/VBox', 'sap/m/FlexItemData', 'sap/m/Label', 'sap/ui/core/ResizeHandler',
               'sap/ui/comp/odata/FieldSelectorModelConverter', 'sap/ui/fl/fieldExt/Access', './FieldSelectorRenderer'
               ],
	function(jQuery, library, Control, FieldController, mLibrary, HBox, VBox, FlexItemData, Label, ResizeHandler,
			FieldSelectorModelConverter, Access, FieldSelectorRenderer) {
	"use strict";
 ...
 */
	...
	if (smartFormDialog) {
		this._oResizeDialogHandlerId = ResizeHandler.register(smartFormDialog, this._handleResizeDialog.bind(this));
	}
	if (this._oScrollView) {
		this._oResizeScrollViewHandlerId = ResizeHandler.register(this._oScrollView, this._handleResizeTable.bind(this));
	}
 ...
 * @param {Object} mBindingPathToFieldListElement Map absolute odata binding paths to the field list elements
 * @param {string} mBindingPathToFieldListElement.bindingPath paths
 * @param {Object} mBindingPathToFieldListElement.fieldListElement field list elements
 * @param {Object} mIdToFieldListElement Map field list element ids to the field list elements
 * @param {string} mIdToFieldListElement.id ID
 * @param {Object} mIdToFieldListElement.fieldListElement field list elements
 * @public
 */
 ...
 */
	...
	this._oHeaderLayout = new HBox("smartFormPersDialogFieldSelectorHeader", {
		direction: FlexDirection.Row
	});
	this._oHeaderLayout.addStyleClass("sapUiCompFieldSelectorHeader");

	this._oHeaderLayoutSearch = new HBox({
		direction:FlexDirection.Row
	});
	this._oHeaderLayoutSearch.addStyleClass("sapUiCompFieldSelectorHeaderSearch");

	this._oResizeHeaderHandlerId = ResizeHandler.register(this._oHeaderLayout, this._handleResizeDialog.bind(this));

	var oEntitySelectionLayout = new HBox({
		direction: FlexDirection.Row,
	...
	oDropDown.setLayoutData(new FlexItemData({
		order: 2,
		growFactor: 1
	}));
	...
	oSearchField.setLayoutData(new FlexItemData({
		growFactor: 1
	}));
 ...
 */
	...
	if (!this._bDisplayFieldExtButton) {
		...
		try {
			var oPromise = Access.getBusinessContexts(oMDA.oModel.sServiceUrl, this._sSelectedKey);
 ...
 */
	...
	if (this._oResizeDialogHandlerId) {
		ResizeHandler.deregister(this._oResizeDialogHandlerId);
	}
	if (this._oResizeHeaderHandlerId) {
		ResizeHandler.deregister(this._oResizeHeaderHandlerId);
	}
	if (this._oResizeScrollViewHandlerId) {
		ResizeHandler.deregister(this._oResizeScrollViewHandlerId);
	}
