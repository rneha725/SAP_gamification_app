...
/* global Promise */
sap.ui.define([
	'jquery.sap.global', './ODataType', 'sap/ui/model/odata/AnnotationHelper'
], function(jQuery, ODataType, AnnotationHelper) {
	"use strict";
	...
	var MetadataAnalyser = function(oResourceRootUri) {
		...
		if (!this.oModel && this._sResourceRootUri) {
			jQuery.sap.log.error("Analyzing metadata cannot be done without an ODataModel!");
			return;
		}
		if (this.oModel) {
	...
	MetadataAnalyser.prototype._parseV4PropertyAnnotations = function(oProperty, oEntityDef, sParentPropertyName) {
		...
		oField.visible = oField.visible && !oField.hidden;
