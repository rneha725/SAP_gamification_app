...
// To avoid loading the sap.chart library in every case it is only "lazy"-loaded on library level and loaded explicitly here:
...
sap.ui.define([
...
], function(jQuery, library, Chart, ChartLibrary, HierarchyDimension, Dimension, TimeDimension, Measure, SegmentedButton, Button, Text, FlexItemData, Item, OverflowToolbar, OverflowToolbarButton, ToolbarSeparator, ToolbarSpacer, VBox, VBoxRenderer, ChartProvider, FilterProvider, SmartVariantManagement, Filter, FilterOperator, PersoUtil, Device, ODataModelUtil, MetadataAnalyser, P13nFilterItem, VizTooltip, UIState, SelectionDetails, SelectionDetailsItem, SelectionDetailsItemLine, Title, FormatUtil, NavigationPopoverHandler, InvisibleText, OverflowToolbarLayoutData, MLibrary, PersoController, ChartWrapper) {
	"use strict";
	...
	SmartChart.prototype._createPopover = function() {
		if (this._oChart) {
			...
			if (!this._oPopover && VizPopover) {
				this._oPopover = new VizPopover({});
			}
			if (this._oPopover) {
				// Make this dynamic for the setter call

				// TODO: get official API to check if already a popover is connected
				var oVizFrame = sap.ui.getCore().byId(this._oChart.getVizUid());
				if (!oVizFrame._onConnectPopover()) {
