...
// });
	...
	FilterProvider.prototype._createControl = function(oFieldViewMetadata) {
		...
		if ((oFieldViewMetadata.displayFormat === "UpperCase") && (oFieldViewMetadata.controlType !== sap.ui.comp.smartfilterbar.ControlType.dropDownList) && oControl.attachChange && oControl.getValue && oControl.setValue) {
	...
	FilterProvider.prototype._createParametersByEntitySetName = function(sEntitySetName, aParameterNames) {
		...
		for (var i = 0; i < aParameterMetadataOData.length; i++) {
			...
			if (aParameterNames.indexOf(aParameterMetadataOData[i].name) >= 0) {
				...
				if (oParameterMetadata && oParameterMetadata.visible && !oParameterMetadata.hidden) {
	...
	FilterProvider.prototype._createAnalyticParameterMetadata = function(oParameterMetadataOData) {
		...
		var oFieldMetadata, sParamPrefix = sap.ui.comp.ANALYTICAL_PARAMETER_PREFIX;
		...
		if (oFieldMetadata) {
			...
			jQuery.sap.log.error("_createAnalyticParameterMetadata issue with property: " + oParameterMetadataOData.name);
		}
					...
					if (sField in mConditionTypeFields) {
						...
						if (oNewValue && (oNewValue.items || oNewValue.ranges)) {
							if (oNewValue.ranges && oNewValue.ranges.length) {
								...
								} else if ((oNewValue.ranges.length === 1) && (oNewValue.ranges[0].operation === "EQ") && !oNewValue.ranges[0].value1 && (typeof oNewValue.ranges[0].value1 === "string")) {
									// BCP: 1770464128
									continue;
								}
							}
