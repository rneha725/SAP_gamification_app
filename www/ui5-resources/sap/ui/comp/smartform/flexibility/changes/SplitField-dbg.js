...
sap.ui.define([
	...
	SplitField.applyChange = function(oChange, oControl, mPropertyBag) {
		...
		for (var i = 0, n = aFields.length; i < n; i++) {
			var bIsSmartField = (oModifier.getControlType(aFields[i]) === "sap.ui.comp.smartfield.SmartField");
			if (i !== iLabelElementIndex) {
				...
				// set label of groupElement if not set by smartField (when valid binding is present)
				if ( !bIsSmartField || (bIsSmartField && oModifier.getProperty(aFields[i], "textLabel") === "") ) {
				...
				// set label to combined groupElement if not set by smartField (when valid binding is present)
				if ( !bIsSmartField || (bIsSmartField && oModifier.getProperty(aFields[i], "textLabel") === "") ) {
					vLabel = oModifier.getProperty(oSourceControl, "label");
					if (vLabel && (typeof vLabel !== "string")){
						oModifier.setProperty(vLabel, "text", sLabelText);
