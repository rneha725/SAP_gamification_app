...
sap.ui.define([
				...
				if (this._oChartViewMetadata) {
					...
					if (oDataPoint) {
						...
						if (oDataPoint) {
							...
							jQuery.sap.log.error("There is no UI.Chart annotation nore DataPoint annotation. We cannot initialize SmartMicroChart.");
						}
					} else {
						jQuery.sap.log.error("There is no UI.Chart annotation nore DataPoint annotation. We cannot initialize SmartMicroChart.");
					}
	...
	SmartMicroChart.prototype._createInnerChartFromDataPoint = function (oDataPoint) {
		...
		if (SmartMicroChartCommons._isBulletVizualizationType(sType)) {
			...
			jQuery.sap.log.error("Only Bullet chart can be initialize with just DataPoint annotation. Type: " + sType + " is not a Bullet chart type.");
			return;
		}
		this.invalidate();
	};
