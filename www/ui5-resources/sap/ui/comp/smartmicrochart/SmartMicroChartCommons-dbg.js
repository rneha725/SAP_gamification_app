...
sap.ui.define("sap/ui/comp/smartmicrochart/SmartMicroChartCommons", [
	...
	Commons._onMetadataInitialized = function() {
		...
		if (Commons._checkChartMetadata.call(this)) {
			...
			if (Commons._checkDataPointAnnotation(this._oDataPointAnnotations, this._oChartViewMetadata)) {
	...
	Commons._checkChartMetadata = function() {
		if (jQuery.isEmptyObject(this._oChartViewMetadata)) {
			if (jQuery.isEmptyObject(this._oDataPointMetadata)) {
				jQuery.sap.log.error("DataPoint annotation must be provided if no chart annotation is missing.");
				return false;
			}
			return true;
		} else {
	 ...
	 * @param {object} oChartAnnotation The chart annotation.
	 * @returns {boolean} True if the annotation check was successful, otherwise false.
	 * @private
	 */
	Commons._checkDataPointAnnotation = function(oDataPointAnnotations, oChartAnnotation) {
		...
		if (Commons._hasMember(oDataPointAnnotations, "Visualization.EnumMember")) {
			...
			if (!Commons._isBulletVizualizationType(sVisualizationType)) {
				jQuery.sap.log.error("The only support visualization types for micro charts are BulletChart and DeltaBulletChart.");
				return false;
			}
		}
		if (jQuery.isEmptyObject(oChartAnnotation) && (sVisualizationType === null)) {
			jQuery.sap.log.error("If only DataPoint annotation is provided the VisualizationType is expected to be set.");
			return false;
		}
