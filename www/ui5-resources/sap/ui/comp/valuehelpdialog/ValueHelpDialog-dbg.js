...
sap.ui.define([
...
], function(jQuery, Dialog, MessageBox, Token, ItemsCollection, Control, DateFormat, NumberFormat, VBox, HBox, FormatUtil, DialogRenderer, 
		uiTable, mTable, Boolean, FlexItemData, Title, Text, Label, Button, ButtonType, MTableColumn, UiTableColumn, Bar, OverflowToolbarLayoutData, Tokenizer, Panel, StandardListItem, IconTabBar, 
		IconTabFilter, mLibrary, InvisibleText, IconPool, HorizontalLayout, Grid, Device, List, Link, P13nFilterPanel, P13nItem, P13nFilterItem, CheckBox, String) {
		"use strict";
	...
	ValueHelpDialog.prototype._createRanges = function() {
		...
		this._oFilterPanel = new P13nFilterPanel(this.getId() + "-filterPanel", {
			...
			addFilterItem: function(oEvent) {
				...
				this._addToken2Tokenizer(params.key, sTokenText, oRange.exclude ? this._oExcludedTokens : this._oSelectedTokens, oRange.keyField);
				this._updateTokenizer();
			}.bind(this),
		...
		if (this._aRangeKeyFields) {
			this._aRangeKeyFields.forEach(function(item) {
				...
				oIDs[item.key]++;
				this._oFilterPanel.addItem(new P13nItem(this.getId() + "-item_" + item.key + oIDs[item.key], {
					columnKey: item.key,
					text: item.label,
					type: item.type,
		...
		if (this._oSelectedRanges) {
			for (var rangeId in this._oSelectedRanges) {
				...
				if (!oIDs[rangeData.keyField]) {
					oIDs[rangeData.keyField] = 0;
				}
				oIDs[rangeData.keyField]++;
				var oFilterItem = new P13nFilterItem(this.getId() + "-item_" + rangeData.keyField + oIDs[rangeData.keyField], {
					key: rangeId,
	...
	ValueHelpDialog.prototype._getBooleanValues = function() {
		if (!this._aBooleanValues) {
			var oBooleanType = new Boolean();
			if (oBooleanType) {
				this._aBooleanValues = [
					"", oBooleanType.formatValue(false, "string"), oBooleanType.formatValue(true, "string")
				];
			}
		}
		return this._aBooleanValues;
	};
			...
			switch (oCurrentKeyField.type) {
				case "numc":
					oFormatter = {
						...
						//						},
						oType: new String({}, oCurrentKeyField.formatSettings)
					};
					break;
				case "numeric":
