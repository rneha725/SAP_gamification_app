 ...
 */
sap.ui.define([
	"./NavError", "./SelectionVariant", "sap/ui/generic/app/library", "sap/ui/base/Object", "sap/ui/model/resource/ResourceModel", "sap/ui/core/UIComponent", "sap/ui/core/routing/HashChanger", "jquery.sap.global"
		...
		constructor: function(oController, sParamHandlingMode) {
			...
			try {
				...
				if (!this.oCrossAppNavService) {
					jQuery.sap.log.error("NavigationHandler: CrossApplicationNavigation is not available.");
		...
		navigate: function(sSemanticObject, sActionName, vNavigationParameters, oInnerAppData, fnOnError) {
			...
			oSupportedPromise.done(function(oTargets) {
				if (oTargets[0].supported) {
					...
					oReturn = oNavHandler._saveAppStateWithImmediateReturn(oObj, fnOnError);
					if (oReturn) {

						// in case of explace navigation wait, till the xapp-state is successfully written
						oXAppStatePromise = bExPlace ? oReturn.promise : oNavHandler._createDummyResolvedPromise();

						oXAppStatePromise.then(function() {
...
// }
		...
		_createDummyResolvedPromise: function() {
			var oMyDeferred = jQuery.Deferred();
			oMyDeferred.resolve("");
			return oMyDeferred.promise();
		}
