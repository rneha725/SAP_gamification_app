	...
	function(jQuery, JSONModel, Filter, FilterOperatorConfig) {
		...
		ConditionModel.prototype.clone = function(sFieldPath) {
			...
			if (oFF) {
				...
				jQuery.sap.log.error("ConditionModel", "clone of ConditionModel for fieldPath '" + sFieldPath + "' failed!");
				return oCM;
			}
			...
			if (bShowMessage) {
				...
				aFields.forEach(function(sFieldPath) {
					if (this._mFieldPath && this._mFieldPath[sFieldPath]) {
						...
						while (oFilterField.getMaxConditions() >= 0 && this.getConditions(sFieldPath).length > oFilterField.getMaxConditions()) {
