			...
			vos.forEach(function(definition) {
				...
				switch (definition.type) {
					case "{00100000-2012-0004-B001-64592B8DB964}": // Spot
						...
						voAggregation = new sap.ui.vbm.Spots(mapId + "-" + definition.id);
						processDragAndDrop();
						break;
					case "{00100000-2012-0004-B001-C46BD7336A1A}": // Route
						...
						voAggregation = new sap.ui.vbm.Routes(mapId + "-" + definition.id);
						processDragAndDrop();
						break;
		...
		if (automation.Call.handler === "CONTEXTMENUHANDLER") {
			...
			if (automation.Call.instance && automation.Call.instance.split('.').length == 2) {
				...
				if (oVos.length > 0) {
					...
					automation.Call.object = that._map().getId() + "-" + automation.Call.object;
				}
			}
	...
	Adapter.prototype._genericGeomapHandler = function(event) {
		...
		if (oEventData.Action && oEventData.Action.object) {
			...
			if (oAction) {
				 ...
				 * But GeoMap only provides pos since that is how the action is setup in GeoMap.js at line # 1417
				 */
