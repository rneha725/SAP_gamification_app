...
sap.ui.define([
	...
	ViewStateManager.prototype.setSelectionStates = function(selectedNodeRefs, unselectedNodeRefs, recursive) {
		...
		var selected = jQuery.sap.unique(recursive ? this._collectNodesRecursively(selectedNodeRefs) : selectedNodeRefs)
		.filter(function(nodeRef) {
			return nodeRef && this._getSelectionFlagState(nodeRef) === false;
		}, this);

		var unselected = jQuery.sap.unique(recursive ? this._collectNodesRecursively(unselectedNodeRefs) : unselectedNodeRefs)
		.filter(function(nodeRef) {
