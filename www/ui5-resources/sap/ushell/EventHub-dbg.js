    ...
    return fnDefine([], function () {
        ...
        function notifySubscriber (oHub, sEvent, oSubscriber, aSubscriberGroup) {
            ...
            return new Promise(function (fnResolve) {
                ...
                var vDeserializedValue = deserialize(oHub.pendingEvents[sEvent]);
                setTimeout(function () {
        ...
        function emit (oHub, sEvent, oData) {
            var sSerializedData = serialize(oData);
        ...
        function last (oHub, sEvent) {
            return deserialize(oHub.pendingEvents[sEvent]);
        }
        ...
        function createChannel (oContract) {
            var oContractClone = deserialize(serialize(oContract));
            var oEventHubData = {
                pendingEvents: { },
                subscribers: { },
                dispatchOperations: { }
            ...
            function setObjectValue (oDef, vObject, aPath, vValue) {
                ...
                return aPath.reduce(function (oCurrentObject, sKey, iIdx) {
                    sPathSoFar += "/" + sKey;  // use sPathSoFar for errors only
                    oDefSoFar = oDef[sKey];

                    if (iIdx === aPath.length - 1) {
                        if (oCurrentObject[sKey] !== null && typeof oCurrentObject[sKey] === "object") {
                            throw new Error("Cannot assign a value to a non-leaf node at path '" + sPathSoFar + "'");
                        }
            ...
            function getObjectValue (oObjectOrArray, aPath) {
                ...
                var oLastItem = aPath.reduce(function (oCurrentObject, sKey) {
                    ...
                    if (!oCurrentObject.hasOwnProperty(sKey)) {
                        throw new Error("The item '" + sKey + "' from path " + sPathSoFar + " cannot be accessed in the object: " + serialize(oCurrentObject));
                    }
            ...
            function findEmittableParentEvents (aParentValues) {
            ...
            function channelEmit (sPath, vData) {
                ...
                // write in the clone
                setObjectValue(oContract, oContractClone, aPath, vData);

                oEventHub.emit(serializePath(aPath), vData);

                // Notify parents as well if any is subscribed.
                //
                // Here we could semantically emit on parent paths, but it
                // would make the channel unnecessarily crowded. As an
                // optimization the emission for later subscribers is made at
                // the time the parent path is subscribed.
                //
                var aParentValues = getParentValues(oContractClone, aPath);
                findEmittableParentEvents(aParentValues).forEach(function (oParentEventInfo) {
                    oEventHub.emit(oParentEventInfo.path, oParentEventInfo.value);
                });
            }
            ...
            function channelLast (sPath) {
                ...
                if (vValueFromOriginalObject === undefined) {
                    // return here, because the kind of clone we do removes all
                    // keys that point to undefined values (i.e., the
                    // getObjectValue on the clone would fail!).
                    return undefined;
                }
                return getObjectValue(oContractClone, aPath);
            }
            ...
            function channelOn (sPath) {
                ...
                if (typeof vLastChangedValue !== "undefined"
                    && (isPrimitive(vLastChangedValue)
                        || !isEmpty(deserialize(serialize(vLastChangedValue))))
                ) {
