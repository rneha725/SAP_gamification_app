...
sap.ui.define(["sap/ui/core/theming/Parameters"], function (Parameters) {
     ...
     *
     * @param {string} sMarkName the name of the perfromance mark
     *
     * @param {boolean} bUseUniqueMark use only the first measurement for a mark's position
     */
    utils.setPerformanceMark = function (sMarkName, bUseUniqueMark) {
        if (performance && performance.mark) {
            if (bUseUniqueMark && performance.getEntriesByName(sMarkName).length != 0) {
                return;
            }
